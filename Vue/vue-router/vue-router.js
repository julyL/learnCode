...
(function (global, factory) {
    ...
    // 返回匹配的routeRecode数组，父路由在前，子路由在后
    ...
    */
    ...
    // 递归遍历route和route的children,将路由信息以特定格式存入pathList、pathMap、nameMap中，方便建立映射关系进行查找
    function addRouteRecord(
        ...
        if (route.alias !== undefined) {
            ...
            // 为别名路由及其所有子节点重新生成routeRecord并加入到pathList、pathMap、nameMap中
            // 例如存在 { path: '/bar', component: Root, alias: '/bar-alias' }
            // 那么会存在2个routeRecord,分布为'/bar'的和'bar-alias'的
    ...
    function createMatcher(
        ...
        // 始终返回一个route对象(通过createRoute创建),这个route对象是根据raw匹配创建的，没有匹配时会返回一个匹配'/'的route对象
        function match(
            raw,           // RawLocation
            currentRoute,  // ?Route
            redirectedFrom // ?Location
            ...
            if (name) {
                ...
                // location.path为需要目标路由的url路径(此时浏览器的url还未改变)
                location.params = {};
                for (var i = 0; i < pathList.length; i++) {
                    var path = pathList[i];
                    var record$1 = pathMap[path];
                    // 通过matchRoute查找定义的路由表中是否有匹配location.path
                    if (matchRoute(record$1.regex, location.path, location.params)) {
                        // 如果有匹配,根据location创建新route对象
    ...
    // 检查path能否通过regex的匹配，并对params对象正确赋值
    function matchRoute(
        regex,
        path,
        params
    ...
    function resolveAsyncComponents(matched) {
        return function (to, from, next) {
            ...
            // 对所有matched的路由组件执行以下回调, 判断是否是异步组件,并等到所有异步组件调用完成后执行next进行下一步
    ...
    // 对matched中的每一个RouteRecord对象,遍历components上所有的属性, 并传入到fn中执行
    ...
    // location为需要切换到的路由对应的Location对象(此时浏览器url还未变化)
    // this.current为当前url对应的route对象
...
`   `       current.matched为 ['/user'的routeRecord,'/user/profile'的routeRecord]
`   `       route.matched为 ['/user'的routeRecord,'/user/posts'的routeRecord]
        */
        var lastRouteIndex = route.matched.length - 1;
        var lastCurrentIndex = current.matched.length - 1;
        // 判断前后路由是否一致
        // route.matched[lastRouteIndex]为最匹配的路由
        if (
    ...
    /*
      按照子路由组件 => 父级路由组件的顺序, 返回beforeRouteLeave数组
      eg: /a/b路由销毁时，会先执行/b路由的beforeRouteLeave,再执行/a路由对应的beforeRouteLeave
      通过传递第4个参数控制父子路由的顺序
    */
    // (deactivated: Array<RouteRecord>): Array<?Function>
    function extractLeaveGuards(deactivated) {
        return extractGuards(deactivated, 'beforeRouteLeave', bindGuard, true)
    }

    // beforeRouteUpdate触发顺序则是 父级路由 => 子级路由
    function extractUpdateHooks(updated) {
        return extractGuards(updated, 'beforeRouteUpdate', bindGuard)
    }

    // 柯里化实现 将guard的this指向instance
            ...
            return guard(to, from, function (cb) {
                if (typeof cb === 'function') {
                    cbs.push(function () {
                        ...
                        // is no longer valid.
                        /*
                          由于可能存在transition动画,导致beforeRouteEnter执行时可能dom还没有挂载。
                          Vue内部会设置定时器检测新组件是否挂载,如果向next内传入了回调函数cb,如下
                            beforeRouteEnter (to, from, next) {
                                next(vm => {
                                    通过 `vm` 访问组件实例,对应这里的match.instances
                                })
                            }     
                        */
    ...
    VueRouter.prototype.init = function init(app /* Vue component instance */) {
        ...
        if (history instanceof HTML5History || history instanceof HashHistory) {
            ...
            // 进行一次当前页面url的修正,如hash模式，index.html会修正为 index.html#/ 
